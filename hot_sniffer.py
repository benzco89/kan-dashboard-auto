...
ספים (פוסט צעיר מ-24ש שחוצה אחד מהם = חם):
  תגובות: רצפה קבועה לפלטפורמה, ≈p98 של 30 יום.
  צפיות / לייקים / שיתופים: פי-2 מ-p90 של 7 הימים האחרונים (מחושב בזמן
...
כיול 2026-07-29 (63 ההתראות של 17 הימים הראשונים מול ההתפלגות בגיליון):
פי-1.5 ורצפות ה-600/1000/400 נתנו 26 התראות בשבוע - כל ריצה שנייה-שלישית.
1.5*p90 הוא בערך p95, ו-16 מ-38 הטריגרים היחסיים נחתו ב-1.5-1.79. פי-2
ורצפות p98 מורידים ל-~6 בשבוע ומשאירים את כל ההתפוצצויות האמיתיות.
...
# רצפות תגובות ≈p98 של 30 יום לכל פלטפורמה (כויל 2026-07-29). הרצפות הקודמות
# (600/1000/400) נגזרו מ-comment_analyzer כפול 2 ולא מההתפלגות עצמה, ושקעו עם
# הזמן לאחוזון 88-93: טיקטוק 400 ישב מתחת ל-p90 של טיקטוק עצמו.
HOT_COMMENTS = {'instagram': 1000, 'facebook': 2000, 'tiktok': 800}
BASELINE_MULT = float(os.environ.get('TEST_MULT') or 2.0)  # "חם" = פי-2 מ-p90
BASELINE_DAYS = 7
YOUNG_HOURS = 24
