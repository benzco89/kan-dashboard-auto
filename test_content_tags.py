...
print("\nהטבלה שנכרתה מהנתונים\n")

# נכרתה 2026-09-06 ע"י mine_programs.py מ-2,564 כיתובים: פריט שנושא גם
# האשטאג של תוכנית וגם שם הוא דוגמה מתויגת. סף: 3 מופעים ו-70% דומיננטיות.
for _person, _program, _n in [
    ...
    check(f"{_person} -> {_program} ({_n})",
          ct.PROGRAM_BY_PERSON.get(_person), _program)

# מפוצל בין שלוש תוכניות (4/4/5) - שיוך לאחת מהן הוא המצאה, לא מדידה.
# הוא מקבל person ותוכנית ריקה, וזה מצב קריא.
check("כתב מפוצל בין תוכניות נשאר בלי תוכנית",
      "אליאור לוי" in ct.PROGRAM_BY_PERSON, False)
check("ובכל זאת מזוהה בשמו",
      ct.tag_item("כתבה (אליאור לוי)", "instagram")["person"], "אליאור לוי")

# הנתונים אמרו "כאן בשש" x3, הערך הידני אומר רשת ב'. ערך שאדם קבע אינו
# נדרס ע"י כרייה - הוכרע 2026-09-06 להשאיר כפי שהוא.
check("ערך ידני גובר על הכרייה", ct.PROGRAM_BY_PERSON["כרמלה מנשה"], "רשת ב׳")

check("חתימה בסוגריים מזהה גם תוכנית דרך הטבלה",
      ct.tag_item("הכתבה של הערב (שרון דוידוביץ)", "instagram")["program"],
      "חדשות הלילה")
